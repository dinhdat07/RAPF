"""
Modified training file with FSA integration.

Changes marked with: # FSA: <description>
"""
...
from continual_clip.losses import contrastive_loss, ClipLoss
from continual_clip.datasets import build_cl_scenarios
import numpy as np

# FSA: Import the patch
from continual_clip.fsa_patch import patch_model_with_fsa
...
def run_class_incremental(cfg, device):
    ...
    # FSA: Apply patch to enable FSA
    if getattr(cfg, 'use_fsa', False):
        model = patch_model_with_fsa(model, cfg)
        print(f"[FSA] Enabled with lambda_anchor={model.lambda_anchor}")
    ...
    for task_id, _ in enumerate(eval_dataset):
        ...
        # FSA: Track FSA loss statistics
        fsa_losses = []

        for i_epoch in range(cfg.epochs):
            ...
            for inputs, targets, task_ids in tqdm_loader:
                ...
                # ============================================
                # Replay sample preparation (unchanged)
                # ============================================
                if task_id > 0:
                    sg_inputs = []
                    sg_targets = []
                ...
                # ============================================
                # FSA: Add function-space anchoring loss
                # ============================================
                ...
                if task_id > 0 and hasattr(model, 'use_fsa') and model.use_fsa:
                    ...
                    fsa_losses.append(fsa_loss.item())
                
                # Total loss
                loss = clip_loss + model.lambda_img * image_aug_loss + loss_hinge
                
                # FSA: Add FSA term to total loss
                if task_id > 0 and hasattr(model, 'use_fsa') and model.use_fsa:
        ...
        # FSA: Log average FSA loss for this task
        if len(fsa_losses) > 0:
            avg_fsa_loss = sum(fsa_losses) / len(fsa_losses)
            print(f"[FSA] Task {task_id} average FSA loss: {avg_fsa_loss:.4f}")
        
        # ============================================
        # Prototype analysis (unchanged)
        # ============================================
        ...
        with open(cfg.log_path, 'a+') as f:
            f.write(json.dumps({
                ...
                'bwt': round(bwt, 2),
                'fwt': round(fwt, 2),
                'avg_fsa_loss': round(sum(fsa_losses) / len(fsa_losses), 4) if len(fsa_losses) > 0 else 0.0  # FSA: Log FSA loss
            }) + '\n')
