...
class ClassIncrementalCLIP(nn.Module):
    ...
    def forward(self, image, ori_ima_f=False, memory_data=None, not_ini=False, edge_sample=None, prompt=False):
        ...
        # image_features = self.adapter(image_features.detach().to(self.dtype)).to(self.clip_type)
        image_features = self.adapter(image_features.detach().to(torch.float32))
